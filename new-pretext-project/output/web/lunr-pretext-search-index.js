...
},
{
  ...
  "body": " Second Solution  Following the first four paragraphs of the First Solution, recall that divides . Denote . Recall that the units digit is so and therefore . We have .  As noted in the First Solution , so has at most digits when expressed in base . So we may always assume . But if , then by the foregoing as desired.  Another way to prove is to first to recall that and . then . But , so . Then the proof proceeds as before.  "
},
{
  "id": "thirdsolution",
  "level": "1",
  ...
  "body": " Third Solution  This solution will prove an explicit formula for the digits in the base expansion of , specifically . Note that once this is established, that . Establishing this fact will finish the proof becuase if , then .  For any with       The proof is by downward induction on . When , the statement is which is true.  Suppose the statement holds for . Then using the induction hypothesis . The last step used the identity .  Notice that is is an integer and is a real number then .  Using this identity further simplifies to This completes the induction step.       "
},
{
  "id": "lemma-digits",
  "level": "2",
